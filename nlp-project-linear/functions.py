...
class T5FineTuner(pl.LightningModule):
  def __init__(self, hparams, train_df, val_df):
    ...
    self.activation_function = torch.nn.Softmax()
  ...
  def forward(
    ...
    x = x[torch.arange(x.shape[0]), (torch.sum(attention_mask, dim = 1) - 1), :]
    x = self.linear(x)
...
class DatasetProcessor(Dataset):
  ...
  def __getitem__(self, index):
    ...
    return {"source_ids": source_ids, "source_mask": src_mask, "labels" : self.targets[index]}
